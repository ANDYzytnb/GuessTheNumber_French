...
# 新的 GitHub API 配置
public_repo_api_url = "https://api.github.com/repos/ANDYzytnb/GuessTheNumber_French/releases/latest"
public_repo_base_url = "https://github.com/ANDYzytnb/GuessTheNumber_French/releases/download"
...
def download_update(latest_version):
    print("正在下载更新...")
    download_url = f"{public_repo_base_url}/{latest_version}/GuessTheNumber-French-{latest_version}.exe"
    try:
        response = requests.get(download_url, stream=True)
        response.raise_for_status()
        filename = f"GuessTheNumber-French-{latest_version}.exe"
...
def select_difficulty():
    print("请选择难度模式：")
    print("1. 简单模式 (0-20)")
    print("2. 自定义模式 (自定义范围，最大可为0-500000)")
    
    while True:
        try:
            choice = int(input("请输入难度选择 (1-2)："))
            if choice == 1:
                return 0, 20, False, None  # 不启用挑战模式
            elif choice == 2:
                return *custom_range(), False, None  # 不启用挑战模式
            elif choice == 9:
                ...
                if password == dev_mode_password:
                    ...
                    return min_range, max_range, True, number_to_guess  # 开启开发者模式
                else:
                    print("密码错误，回到普通模式。")
...
# 主游戏运行函数
def play_game():
    check_update = input("是否检查更新？(y/n)：").lower()
    if check_update == 'y':
